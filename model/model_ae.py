...
class BiGruEncoder(nn.Module):
    ...
    def __init__(self, embedding_hidden_dim, gru_hidden_dim, num_hidden_layer, dropout_p):
    ...
    def forward(self,
                encoder_embedded):  # encoder_embedded = (B x L x h)

        batch_size, max_len, _ = encoder_embedded.shape
        encoder_outputs, encoder_hidden = self.gru_layer(encoder_embedded)  # (B x L x 2H), (2*N x B x H)

        # Sum forward and backward outputs   (B x L x 2H)  =>  (B x L x H)
